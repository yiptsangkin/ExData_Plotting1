...
value2<-ts(file$Voltage) #将Voltage取时间序列
plot(value2,axes=F,xlab="datetime",ylab="Voltage") #画出不包含边框的图和坐标的图
box() #添加边框
axis(side=1,at=c(0,length(file$Voltage)/2,length(file$Voltage)),lab=c("Thu","Fri","Sat")) #添加横坐标并标出星期区间
axis(side=2) #添加纵坐标
...
axis(side=2) #添加纵坐标
legend("topright",legend=c("Sub_metering_1","Sub_metering_2","Sub_metering_3"),lwd=1,col=c("black","red","blue"),bty="n") #添加标识

#图四

value3<-ts(file$Global_reactive_power) #将Global_reactive_power取时间序列
plot(value3,axes=F,xlab="datetime",ylab="Global_reactive_power") #画出不包含边框的图和坐标的图
box() #添加边框
axis(side=1,at=c(0,length(file$Global_reactive_power)/2,length(file$Global_reactive_power)),lab=c("Thu","Fri","Sat")) #添加横坐标并标出星期区间
axis(side=2) #添加纵坐标